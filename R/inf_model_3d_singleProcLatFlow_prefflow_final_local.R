...
inf_model_3d_singleProcLatFlow_prefflow_final_local = function(param_1, param_2, param_3){
...
latflow_fac = param_vec[2]
## factor of cell width for prefflow cell structure
fac_prefcel_width = param_vec[3]
...
if(use_scenario == "prefflow"){
    ...
    tsx = dplyr::mutate(Irrigation_grid,
          ...
          dplyr::mutate(infProcess = rep(prefflow_cell_process_zlayer, length(zlayers))) %>%
          dplyr::mutate(nlayer = rep(prefflow_cell_nlayer_zlayer, length(zlayers))) %>%
          dplyr::mutate(dtheta = rep(prefflow_cell_dtheta_zlayer, length(zlayers))) %>%
          dplyr::mutate(sat = rep(prefflow_cell_sat_zlayer, length(zlayers))) %>%
          dplyr::mutate(cnt = 1) %>%
    ...
    tsx = dplyr::mutate(Irrigation_grid,
          # column = rep(gridcells$ncell, length(zlayers))) %>%
          column = rep(1:num_cell, length(zlayers))) %>%
          # dplyr::mutate( prefflow_cell = rep(prefflow_cell_code_zlayer, length(zlayers))) %>%
          dplyr::mutate(cnt = 1) %>%
          dplyr::mutate(datetime = 0) %>%
          dplyr::mutate(value = 0) %>%
          dplyr::mutate(prevalue = 0)
...
kge_value = mNSE(infiltration_gmod$gmod, gravity_timesteps$gmod[1:length(infiltration_gmod$gmod)])
