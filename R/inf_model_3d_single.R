#' @title Hydro-gravimetric model (incl. lateral flow), for dynamic infiltration processes WITHOUT macropore layer(s)
#'
...
inf_model_3d_single = function(param_vec){

# ####################
# ## debugging
# param_vec = c(dtheta_other_start, mdepth_start, latflow_fac_start)
# ####################
...
layer_params = data.frame(Depth = zlayers,
                         infProcess = c(rep("none", mdepth_layer), rep("other",(length(zlayers) - mdepth_layer))),
                         nlayer = c(rep(10000,mdepth_layer), seq(1,length.out=(length(zlayers) - mdepth_layer))),
                         dtheta = c(rep(0,mdepth_layer), rep(dtheta_other,(length(zlayers) - mdepth_layer)))
              )
}
## perched water table
if(inf_dyn == 2){
layer_params = data.frame(Depth = zlayers,
                         infProcess = c(rep("none", mdepth_layer), rep("other", layer_between_processes), rep("none",(length(zlayers) - pdepth_layer))),
                         nlayer = c(rep(10000,mdepth_layer), rev(seq(1,length.out=layer_between_processes)), rep(10001, (length(zlayers) - pdepth_layer))),
                         dtheta = c(rep(0,mdepth_layer), rep(dtheta_other, layer_between_processes), rep(0,(length(zlayers) - pdepth_layer)))
              )
}
## by-pass flow
if(inf_dyn == 3){
layer_params = data.frame(Depth = zlayers,
                         infProcess = c(rep("none", mdepth_layer), rep("none", layer_between_processes), rep("other",(length(zlayers) - pdepth_layer))),
                         nlayer = c(rep(10000,mdepth_layer), rep(10001, layer_between_processes), seq(1,length.out=(length(zlayers) - pdepth_layer))),
                         dtheta = c(rep(0,mdepth_layer), rep(0, layer_between_processes), rep(dtheta_other,(length(zlayers) - pdepth_layer)))
              )
}
# old version
# layer_params = data.frame(Depth = zlayers,
...
tsx = dplyr::mutate(tsx,# value_macro = ifelse(infProcess == "macro" & nlayer == layerfill, prevalue + (distrWater / cellsLayerColumn), prevalue)) %>%
      ...
      ## normal filing
      # dplyr::mutate(
                    value_other = ifelse(infProcess == "other" & nlayer == layerfill, prevalue + (distrWater / cellsLayerColumn) * vertflow_fac, prevalue)) %>%
      ...
      dplyr::mutate(value = value_other)
####################
...
# except for the macro pores: NO MORE MACRO PORES IN THIS MODEL TYPE !!
tsx$belowSatnLayer = paste(tsx$cell_id,"_", (tsx$nlayer))
tsx$aboveSat[which(tsx$belowSatnLayer %in% cells_sat$belowSatnLayer)] = T
...
cellnums_dynamic = dplyr::mutate(layerfilling, #ncells = ifelse(infProcess == "macro" & layerfill < 2, mdepth * 10 +1, 0)) %>%
                   # dplyr::mutate(ncells = ifelse(infProcess == "other", 1, ncells)) %>%
                                 ncells = ifelse(infProcess == "other", 1, 0)) %>%
