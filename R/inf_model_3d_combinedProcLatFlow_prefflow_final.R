...
inf_model_3d_combinedProcLatFlow_prefflow_final = function(param_1, param_2, param_3, param_4, param_5){

# ####################
# ## debugging
# param_vec = c(dtheta_macro_start, dtheta_other_start, mdepth_start, latflow_fac_start)
#
# param_1 = as.numeric(dtheta1_start)
# param_2 = as.numeric(pdepth_start)
# param_3 = as.numeric(latflow_fac_start)
# ####################
...
param_vec = c(param_1, param_2, param_3, param_4, param_5)
...
dtheta = param_vec[1]
dtheta2 = param_vec[2]
pdepth = round(param_vec[3],1)
pdepth2 = round(pdepth - 0.1,1)
# lateral flow factor (seperate into lateral and vertical flow)
latflow_fac = param_vec[4]
## factor of cell width for prefflow cell structure
fac_prefcel_width = param_vec[5]
...
# ####################
# NEW for prefflow:
# there has to be a "secondary" cell grid of quadratic cells
# which define "flow" or "no flow" for preferential flow process
# factor for "every <which> cell should be no flow?
# fac_prefcel_width = 5
# create vector of flow/no flow properties for ONE horizonal celllayer
# prefflow_cell_code_zlayer = rep(c(rep(0, each=fac_prefcel_width), rep(1, each = fac_prefcel_width)), length.out = num_cell)
#
if(use_scenario == "prefflow" & use_scenario2 == "wfa"){
    #
    # later to be replaced by factor for optimization !!
    # fac_prefcel_width = 5
    # create vector of flow/no flow properties for ONE horizonal celllayer
    ## process type
    # first "row" of unit blocks, e.g. 0...1...0...1
    prefflow_cell_process_row = rep(c(rep("other", each=fac_prefcel_width), rep("none", each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_process_unit_even = rep(prefflow_cell_process_row, fac_prefcel_width)
    # second "row" of unit blocks, e.g. 1...0...1...0
    prefflow_cell_process_row = rep(c(rep("none", each=fac_prefcel_width), rep("other", each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_process_unit_odd = rep(prefflow_cell_process_row, fac_prefcel_width)
    # combine both to Schachbrett
    prefflow_cell_process_zlayer = rep(c(prefflow_cell_process_unit_even, prefflow_cell_process_unit_odd), length.out = num_cell)
    # procedd for wfa
    wfa_cell_process_zlayer = rep("other", length.out = num_cell)
    ## nlayer
    prefflow_cell_nlayer_row = rep(c(rep(1, each=fac_prefcel_width), rep(10001, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_nlayer_unit_even = rep(prefflow_cell_nlayer_row, fac_prefcel_width)
    prefflow_cell_nlayer_row = rep(c(rep(10001, each=fac_prefcel_width), rep(1, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_nlayer_unit_odd = rep(prefflow_cell_nlayer_row, fac_prefcel_width)
    prefflow_cell_nlayer_zlayer = rep(c(prefflow_cell_nlayer_unit_even, prefflow_cell_nlayer_unit_odd), length.out = num_cell)
    # nlayer for wfa
    wfa_cell_nlayer_remainingZlayer = rep(seq(1,length.out=(length(zlayers) - pdepth_layer)), each = num_cell)
    ## dtheta
    prefflow_cell_dtheta_row = rep(c(rep(dtheta, each=fac_prefcel_width), rep(0, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_dtheta_unit_even = rep(prefflow_cell_dtheta_row, fac_prefcel_width)
    prefflow_cell_dtheta_row = rep(c(rep(0, each=fac_prefcel_width), rep(dtheta, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_dtheta_unit_odd = rep(prefflow_cell_dtheta_row, fac_prefcel_width)
    prefflow_cell_dtheta_zlayer = rep(c(prefflow_cell_dtheta_unit_even, prefflow_cell_dtheta_unit_odd), length.out = num_cell)
    # dtheta2 for wfa
    wfa_cell_dtheta_zlayer = rep(dtheta2, length.out = num_cell)
    # also hard code saturation state (at beginning)
    ## sat
    prefflow_cell_sat_row = rep(c(rep(F, each=fac_prefcel_width), rep(T, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_sat_unit_even = rep(prefflow_cell_sat_row, fac_prefcel_width)
    prefflow_cell_sat_row = rep(c(rep(T, each=fac_prefcel_width), rep(F, each = fac_prefcel_width)), length.out = length(unique(Irrigation_grid$x)))
    prefflow_cell_sat_unit_odd = rep(prefflow_cell_sat_row, fac_prefcel_width)
    prefflow_cell_sat_zlayer = rep(c(prefflow_cell_sat_unit_even, prefflow_cell_sat_unit_odd), length.out = num_cell)
    # at for wfa
    wfa_cell_sat_zlayer = rep(F, length.out = num_cell)
    #
    tsx = dplyr::mutate(Irrigation_grid,
          # column = rep(gridcells$ncell, length(zlayers))) %>%
          column = rep(1:num_cell, length(zlayers))) %>%
          dplyr::mutate(infProcess = c(rep(prefflow_cell_process_zlayer, pdepth_layer), rep(wfa_cell_process_zlayer,(length(zlayers) - pdepth_layer)))) %>%
          dplyr::mutate(nlayer = c(rep(prefflow_cell_nlayer_zlayer, pdepth_layer), wfa_cell_nlayer_remainingZlayer)) %>%
          dplyr::mutate(dtheta = c(rep(prefflow_cell_dtheta_zlayer, pdepth_layer), rep(wfa_cell_dtheta_zlayer,(length(zlayers) - pdepth_layer)))) %>%
          dplyr::mutate(sat = c(rep(prefflow_cell_sat_zlayer, pdepth_layer), rep(wfa_cell_sat_zlayer,(length(zlayers) - pdepth_layer)))) %>%
          dplyr::mutate(cnt = 1) %>%
...
## single scenarios
# this is done in another file !!
print("For single scenarios, please modify the scenario call settings in inf_model_* file!")
message("For single scenarios, please modify the scenario call settings in inf_model_* file!")
}
...
## ! this is designed, after modifying version of inf_model_3d_macro.R

## Wfa
# if(use_scenario == "wfa"){
# layer_params = data.frame(Depth = zlayers,
#                          infProcess = c(rep("other", length(zlayers))),
#                          nlayer = c(seq(1, length.out = length(zlayers))),
#                          dtheta = c(rep(dtheta, length(zlayers)))
#               )
# }
# ## perched water table
# if(use_scenario == "perched"){
# layer_params = data.frame(Depth = zlayers,
#                          infProcess = c(rep("other", pdepth_layer), rep("none",(length(zlayers) - pdepth_layer))),
#                          nlayer = c(rev(seq(1, length.out = pdepth_layer)), rep(10001, (length(zlayers) - pdepth_layer))),
#                          dtheta = c(rep(dtheta, pdepth_layer), rep(0, (length(zlayers) - pdepth_layer)))
#               )
# }
# ## by-pass flow
# if(use_scenario == "bypass"){
# layer_params = data.frame(Depth = zlayers,
...
# layer_params = data.frame(Depth = zlayers,
#                          infProcess = c(rep("macro", pdepth_layer), rep("none", (length(zlayers) - pdepth_layer))),
#                          nlayer = c(rep(1, pdepth_layer), seq(10001, length.out = (length(zlayers) - pdepth_layer))),
#                          dtheta = c(rep(dtheta, pdepth_layer), rep(0, (length(zlayers) - pdepth_layer)))
#               )
# }
# #
# ## tag cells with layer parameters
# if(use_scenario != "prefflow"){
...
tsx = inner_join(tsx, Intensity_distribution) %>%
      ...
      # this is now already implemented in aboves domain setup !!
      #to secure that no flow really gets no water !!
      # mutate(sat = F) %>%
      mutate(aboveSat = F) %>%
      mutate(value_lat = NA) %>%
...
write.table(dplyr::select(tsx, x, y, z, Depth, value, datetime), 
            file=paste0(output_dir, "model_output/raw/tsx_", n_param, "_TS", formatC(i, width=3, flag="0"), ".txt"), sep="\t", row.names=F, col.names=T, append=F)
}else{
write.table(dplyr::select(tsx, x, y, z, Depth, value, datetime),
            file=paste0(output_dir, "model_output/raw/tsx_", n_param, "_TS", formatC(i, width=3, flag="0"), ".txt"), sep="\t", row.names=F, col.names=F, append=F)
}
...
cellnums_dynamic = dplyr::mutate(layerfilling, ncells = ifelse(infProcess == "macro" & layerfill < 2, pdepth_layer, 0)) %>%
...
# 
# ggplot(tsx_plot, aes(x=x,y=y, color = value)) +
#     geom_point() 
...
## save data
save(Infiltration_model_results, file=paste0(output_dir, "Infiltration_model_output_", n_param, ".rData"))
...
#save gsignal
save(infiltration_gmod, file=paste0(output_dir, "GravityResponse_Infiltration_model_", n_param, ".rData"))
